...
import AuthService from "../services/AuthService"; // Hoặc đường dẫn tương ứng với file của bạn
...
const HomePage = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        const user = await AuthService.getProfile(); // Gọi API để lấy thông tin người dùng
        setCurrentUser(user);
        console.log("User profile từ API:", user);
      } catch (error) {
